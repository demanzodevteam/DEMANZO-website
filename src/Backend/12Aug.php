...
add_action('rest_api_init', function () {
    register_rest_route('custom/v1', '/client-titles', [
        'methods' => 'GET',
        'callback' => function () {
            $clients = get_posts([
                'post_type' => 'client',
    ...
    // FRONT IMAGE handling
    $front = get_field('front_image', $post->ID);
    $front_url = '';

    if (is_array($front) && isset($front['url'])) {
        $front_url = $front['url'];
    } elseif (is_numeric($front)) {
        $front_url = wp_get_attachment_url($front);
    } elseif (is_string($front)) {
        $front_url = $front;
    }

    // Convert full URL to relative
    $front_url = str_replace(site_url() . '/', '', $front_url);

    // BACK IMAGE (same logic if needed)
    $back = get_field('back_image', $post->ID);
    $back_url = '';

    if (is_array($back) && isset($back['url'])) {
        $back_url = $back['url'];
    } elseif (is_numeric($back)) {
        $back_url = wp_get_attachment_url($back);
...
add_action('rest_api_init', function () {
    register_rest_route('myapi/v1', '/hundred-posts', array(
        'methods'  => 'GET',
        'callback' => 'myapi_get_hundred_posts',
    ));
});

// Callback function for the custom endpoint
function myapi_get_hundred_posts(WP_REST_Request $request) {
    $args = array(
        'post_type'      => 'post',
        'posts_per_page' => 100, // Always 100 posts
        'paged'          => $request->get_param('page') ?: 1, // Optional page param
    );

    $query = new WP_Query($args);

    // Collect posts in same format as default REST
    $data = [];
        ...
        'callback' => function () {
            ...
            $data = array_map(function ($post) {
                return [
                    ...
                    'content'  => wp_strip_all_tags(get_field('content', $post->ID)),
                ];
            }, $testimonials);
...
function get_custom_page_data_by_slug($data) {
    ...
    // Properly get and filter full content
    remove_filter('the_content', 'wp_trim_excerpt', 10);
    ...
    register_rest_field(
        ...
        [
            'get_callback' => function( $object ) {
                ...
                $seo_permalink = get_post_meta( $post_id, 'rank_math_permalink', true );

                return [
                    'title'       => $seo_title ?: get_the_title( $post_id ), // fallback to WP title
                    'permalink'   => $seo_permalink ?: get_permalink( $post_id ), // fallback to WP permalink
...
// ---------------
 ...
 add_action('rest_api_init', function () {
    register_rest_route('custom/v2', 'post_details', [
        'methods'  => 'GET',
        'callback' => function () {
            $posts = get_posts([
                'post_type'      => 'post',
            ...
            return array_map(function ($post) {
                ...
                $alt_text  = $image_id ? get_post_meta($image_id, '_wp_attachment_image_alt', true) : null;
...
add_action('rest_api_init', function () {
    register_rest_route('custom/v1', '/all-posts', [
        'methods'  => 'GET',
        'callback' => 'demanzo_get_all_blogs',
        'permission_callback' => '__return_true',
    ]);
});

function demanzo_get_all_blogs($request) {
    $args = [
        'post_type'      => 'post',
        'posts_per_page' => -1, // ✅ No limit, return ALL published posts
        'post_status'    => 'publish',
    ];
    ...
    foreach ($query->posts as $post) {
        ...
        $categories = get_the_category($post->ID);
        $category_name = (!empty($categories) && isset($categories[0]->name)) ? $categories[0]->name : null;
        ...
        $posts[] = [
            ...
            'image'    => $image_url,
            'alt'      => $alt_text,
            'link'     => wp_parse_url(get_permalink($post), PHP_URL_PATH), // ✅ relative link
            'category' => $category_name,
        ];
    }
...
add_action('rest_api_init', function () {
    register_rest_route('custom/v1', '/blog/details', [
        'methods'  => 'GET',
    ...
    foreach ($query->posts as $post) {
        ...
        $posts[] = [
            'title'    => get_the_title($post),
                    ...
                    'image'    => $image_url,
                    'alt'      => $alt_text,
                    'link' => str_replace('/demanzo', '', wp_parse_url(get_permalink($post), PHP_URL_PATH)),
                    'category' => $category_name,
        ];
    }
