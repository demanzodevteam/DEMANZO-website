...
export default function ContactUsFormNew() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Hidden CF7 fields (update IDs with your actual ones)
    formData.append("_wpcf7", "1153");
    formData.append("_wpcf7_version", "5.9.4");
    formData.append("_wpcf7_locale", "en_US");
    formData.append("_wpcf7_unit_tag", "wpcf7-f1153-p12-o1");
    formData.append("_wpcf7_container_post", "12");

    try {
      const res = await fetch(`${FORM_URL}1153/feedback`, {
        method: "POST",
        body: formData,
      });
  ...
  const formFields = [
    { name: "text-517", label: "Your Name", type: "text" },
    { name: "email-875", label: "Email", type: "email" },
    { name: "textarea-785", label: "Message", type: "textarea" },
  ];
    ...
    if (type === "textarea") {
      return (
        ...
        >
          ...
          />
          <label className="absolute left-4 top-4 text-[#050607] text-[14px] font-[400] transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-[16px] peer-placeholder-shown:text-gray-500 peer-focus:top-1 peer-focus:text-[12px] peer-focus:text-black">
            {label}
          </label>
        </motion.div>
      );
    ...
    <>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {renderInput("textarea-785", "textarea", "Message")}
