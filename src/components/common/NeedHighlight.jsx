...
const NeedHighlight = ({ SectionData }) => {
  return (
    <section className="px-4 md:px-10 lg:px-[var(--demanzo-section-spacing)] py-[var(--demanzo-section-spacing)] text-center bg-[#F8FDFF]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:px-30 mt-10">
        {SectionData?.list_items[0]?.map((card, index) => {
          const isOdd = index % 2 === 0;
          return (
            <div
              key={index}
              className={`group shadow-md rounded-xl px-8 py-10 flex flex-col items-center space-y-4 transition max-w-[360px] mx-auto
          ${
            isOdd
